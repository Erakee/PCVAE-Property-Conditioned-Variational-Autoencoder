...
class Decoder(torch.nn.Module):
    ...
    def forward(self, latent_vec, enthalpy, inp, freerun=False, randomchoose=True):
        ...
        if not freerun:
            ...
            X = X_latent
            # X = torch.concat(
            #     [X_latent, torch.zeros((latent_vec.shape[0], 1, self.con_dims), dtype=torch.float32, device=self.device)],
            #     dim=2)
    ...
    def trainModel(self, dataloader, encoderOptimizer, decoderOptimizer, encoderScheduler, decoderScheduler, KLD_alpha,
        ...
        for epoch in range(1, nepoch + 1):
            ...
            for nBatch, (X, enthalpy) in enumerate(dataloader, 1):
                ...
                pred_y = self.decoder(latent_vec, enthalpy, X)  # (512, 128,17)  # 对比时改成latent_vec
                ...
                predicted_smiles = tokenizer.getSmileshot(predicted_indices)  # list 512
